    ...
    try {
        ...
        if (type === 0) {
            if (ids.length === 0) {
                ...
                re = await AddQuery('announces', { 'user_id': jsonData['rooms.user_id'], 'for_id': id, "message": "Đã đến hạn đóng tiền phòng vui lòng thanh toán hóa đơn của bạn!" });
            }
            else {
        ...
        if (type == 1) {//theo han
            ...
            re = await UpdateQuery('room_rents', { "room_id": jsonData['rooms.id'], "is_active": 1 }, { "user_id": id });
            console.log(re)
