...
export default function RoomService() {
    ...
    useEffect(() => {
        GetFetch('room-service/'+dataContext?.id, (data: RoomServiceType[]) => setList2(data), context?.data)
    }, [dataContext?.id])
    ...
    async function handleAdd() {
        ...
        PostFetch('room-service/'+dataContext?.id,newObject,(data:any)=>{
            let x=list2?.find(item=>item.id===object?.service_id)
            if(x===undefined)
    ...
    async function handleDelete(id:number){
        ...
        if(!result || !id)
            return
        DeleteFetch('room-service/'+id,(data:any)=>{
